        ...
        public void cascadeBoard1()
        {
            for (int i = matrixBoard.Count() - 2; i >= 0; i--)
            {
                for (int j = matrixBoard[i].Count() - 1; j >= 0; j--)
                {
                    ...
                    {
                        matrixBoard[auxY + 1][auxX].food.Type = matrixBoard[auxY][auxX].food.Type;
                        matrixBoard[auxY + 1][auxX].setIcon("Sprites/Placeholders_Prototype/food" + matrixBoard[auxY][auxX].food.Type);
                        matrixBoard[auxY + 1][auxX].food.imgIcon.setX(matrixBoard[auxY + 1][auxX].background.getX());
                        matrixBoard[auxY + 1][auxX].food.imgIcon.setY(matrixBoard[auxY + 1][auxX].background.getY());
        ...
        public void fillSpaces()
        {
            ...
            for (int i = 0; i < boardWidth; i++)
            {
                for (int j = 0; j < empties[i]; j++)
                {
                    ...
                    matrixBoard[j][i].food.imgIcon.setY(matrixBoard[j][i].background.getY());
                }
